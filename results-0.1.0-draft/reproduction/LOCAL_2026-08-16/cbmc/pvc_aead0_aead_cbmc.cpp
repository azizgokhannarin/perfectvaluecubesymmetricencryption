...
constexpr std::uint64_t kMaximumEncodedLength = (~static_cast<std::uint64_t>(0));
...
std::uint64_t checked_u64_length(std::size_t size, const char* field) {
    if constexpr (sizeof(std::size_t) > sizeof(std::uint64_t)) {
        if (size > static_cast<std::size_t>(kMaximumEncodedLength)) {
            throw std::length_error("u64 framing limit");
        }
    }
...
void validate_authentication_context_length(std::size_t associated_data_size) {
    if (associated_data_size > (~static_cast<std::size_t>(0)) - kAuthFixedSize) {
...
std::vector<std::uint8_t> apply_keystream(const Key256& key,
    ...
    if constexpr (sizeof(std::size_t) > sizeof(std::uint64_t)) {
        const auto maximum_blocks = static_cast<std::size_t>((~static_cast<std::uint64_t>(0))) + 1U;
    ...
    for (std::size_t block = 0U; block < blocks; ++block) {
        ...
        const std::vector<std::uint8_t> framed = frame_stream_block(nonce, counter, tag_size);
        const pvc1::ResearchOutput stream = pvc1::research_keyed_return_output_a2(key, framed);
...
SealedMessage seal(const KeyPair512& keys,
    ...
    const std::vector<std::uint8_t> context = frame_authentication_context(nonce, associated_data, tag_size);
...
std::optional<std::vector<std::uint8_t>> open(
    ...
    if (!is_supported_tag_size(supplied_tag.size())) return std::optional<std::vector<std::uint8_t>>();
    (void)checked_u64_length(ciphertext.size(), "ciphertext");
    const TagSize tag_size = tag_size_from_bytes(supplied_tag.size());
    const std::vector<std::uint8_t> context = frame_authentication_context(nonce, associated_data, tag_size);
    ...
    return std::optional<std::vector<std::uint8_t>>(apply_keystream(keys.encryption_key, nonce, ciphertext, tag_size));
}
