...
pvcaead0::TagSize parse_tag(const std::string& bits) {
    if (bits == "128") return pvcaead0::TagSize::Bits128;
    if (bits == "192") return pvcaead0::TagSize::Bits192;
    if (bits == "256") return pvcaead0::TagSize::Bits256;
...
int main(int argc, char** argv) {
    try {
        ...
        while (std::getline(input, line)) {
            ...
            pvcaead0::KeyPair512 keys{fixed<32>(f[1]), fixed<32>(f[2])};
            ...
            if (sealed.ciphertext != expected_ciphertext || sealed.tag != expected_tag) {
                throw std::runtime_error("vector mismatch: " + f[0]);
            }
            const auto opened = pvcaead0::open(keys, nonce, ad, sealed.ciphertext, sealed.tag);
            if (!opened.has_value() || *opened != plaintext) throw std::runtime_error("open mismatch: " + f[0]);
            ++count;
        }
        if (count != 48U) throw std::runtime_error("expected 48 vectors");
        std::cout << "vectors=" << count << " status=ok\n";
        return 0;
