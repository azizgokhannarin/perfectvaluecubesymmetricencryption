...
int main(int argc, char** argv) {
    try {
        if (argc != 2) throw std::invalid_argument("usage: pvc-mac0-vector-corpus-tests FILE");
        ...
        while (std::getline(input, line)) {
            ...
            const auto tag_size = pvcmac0::tag_size_from_bytes(static_cast<std::size_t>(std::stoull(fields[4])));
            const auto expected = parse_hex(fields[5]);
            const auto actual = pvcmac0::compute_tag(key, context, message, tag_size);
            if (actual != expected) throw std::runtime_error("corpus vector mismatch at index " + fields[0]);
            ++vectors;
        }
        if (vectors != 48U) throw std::runtime_error("unexpected corpus vector count");
        std::cout << "PASS 48/48 PVC-MAC-0 corpus vectors\n";
        return 0;
